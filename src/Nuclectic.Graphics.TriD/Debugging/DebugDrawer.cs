...
namespace Nuclectic.Graphics.TriD.Debugging
{
	...
	public class DebugDrawer : Drawable, IDebugDrawingService
	{
		...
		public void DrawSolidBox(Vector3 min, Vector3 max, Color color)
		{
			...
			int proposedEnd = this.triangleIndex + VertexCount;
			if ((proposedEnd > this.lineIndex)
				|| (proposedEnd > MaximumDebugVertexCount))
			{
				this.overflowed = true;
				return;
			}
		...
		public void DrawString(Vector2 position, string text, Color color) { this.queuedStrings.Add(new QueuedString(text, position, color)); }
