...
namespace Nuclectic.UserInterface.Controls
{
	...
	internal class ParentingControlCollection : Collection<Control>, IHasSettableScreen
	{
		...
		public ParentingControlCollection(Control parent) { this.parent = parent; }
    ...
    internal void MoveToEnd(int controlIndex) {
      ...
      base.Add(control);
		...
		private void unassignParent(Control item) { item.SetParent(null); }

		/// <summary>Sets up the parentage on the specified item</summary>
		/// <param name="item">Item to be parented</param>
		private void assignParent(Control item) { item.SetParent(this.parent); }
