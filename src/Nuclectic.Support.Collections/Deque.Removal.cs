	...
	partial class Deque<TItem>
	{
		...
		public void Clear()
		{
			...
			else
			{
				...
				for (
					int index = this.firstBlockStartIndex;
					index < this.lastBlockEndIndex;
					++index
					)
				{
					this.blocks[0][index] = default(TItem);
		...
		public void RemoveFirst()
		{
			...
			if (this.firstBlockStartIndex >= this.blockSize)
			{
				...
				if (this.count > 1)
				{
					// Still more blocks in queue, remove block
