...
using Nuclectic.UserInterface.Controls;
...
namespace Nuclectic.UserInterface
{
	...
	public class Screen : IScreen
	{
		...
		public float Width { get { return this.size.X; } set { this.size.X = value; } }

		/// <summary>Height of the screen in pixels</summary>
		public float Height { get { return this.size.Y; } set { this.size.Y = value; } }

		/// <summary>Control responsible for hosting the GUI's top-level controls</summary>
		public Control Desktop { get { return this.desktopControl; } }
				...
				case Command.Right:
				{
					...
					Control nearestControl = null;
					{
						...
						for (int index = 0; index < siblings.Count; ++index)
						{
							...
							if (!ReferenceEquals(sibling, focusedControl)
								&& canControlGetFocus(sibling))
							{
		...
		public void InjectMouseMove(float x, float y) { this.desktopControl.ProcessMouseMove(this.size.X, this.size.Y, x, y); }
		...
		public Control FocusedControl
		{
			get
			{
				...
				if ((current != null)
					&& ReferenceEquals(current.Screen, this))
				{
					return current;
				}
				else
			...
			)
		{
			...
			else
			{
				...
				if (direction == Command.Left)
				{
					...
					if ((closestPointX > sideX)
						&& (leavesTop || leavesBottom))
					{
						return float.NaN;
					}
