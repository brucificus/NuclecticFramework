		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			else
			{
				...
				while (this.activeReadStreamIndex <= lastStreamIndex)
				{
					...
					{
						// Otherwise, go to the next stream in the chain
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			if (this.allStreamsCanSeek)
			{
				...
				while (remaining > 0)
				{
					...
					{
						// We're writing into a stream that's followed by another stream
			...
			{
				// Seeking not supported, append everything to the last stream
