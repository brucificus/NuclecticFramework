	...
	public class ExtendedGamePadStateTest
	{
		...
		public void AccessingInvalidAxisCausesException()
		{
			...
			Assert.Throws<ArgumentOutOfRangeException>(
													   delegate() { state.GetAxis(ExtendedAxes.X | ExtendedAxes.Y); }
				);
		}
		...
		public void AccessingInvalidSliderCausesException()
		{
			var state = new ExtendedGamePadState();
			Assert.Throws<ArgumentOutOfRangeException>(
													   delegate() { state.GetSlider(ExtendedSliders.Slider1 | ExtendedSliders.Slider2); }
				);
		}
		...
		[Ignore("Will fail until a fix for https://github.com/mono/MonoGame/issues/3070 is released")]
